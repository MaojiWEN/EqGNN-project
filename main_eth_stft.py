...
parser.add_argument('--model_save_dir', type=str, default='eth_ucy/STFT',
                    help='Name of the subset.')
...
def log_initial_lr(log_path, lr):
    with open(log_path, 'a') as log_file:
        log_file.write(f"Starting experiment with Learning Rate = {lr:.5f}\n")
...
def main():
    ...
    model = STFT(num_past=args.past_length, num_future=args.future_length, in_node_nf=1, in_edge_nf=1, hidden_nf=args.channels
                  , fft=True, eat=True, device=device, n_layers=2, n_nodes=num_nodes, with_mask=True)

    optimizer = optim.Adam(model.parameters(), lr=args.lr, weight_decay=args.weight_decay)

    log_dir = os.path.join(os.getcwd(), 'log', 'stft')  # 日志文件存储目录
    log_path = create_log_file(log_dir, args.subset)

    log_initial_lr(log_path, args.lr)
    ...
    for epoch in range(0, args.epochs):
        ...
        train(model, optimizer, epoch, loader_train, lr = lr_now)
...
def train(model, optimizer, epoch, loader, backprop=True, lr = 0.1):
    if backprop:
        model.train()
    else:
        model.eval()
    ...
    for batch_idx, data in enumerate(loader):
        if data is not None:
            # 获取数据
            h = data.h.to(model.device)  # [batch_size * num_nodes, num_past, 1]
    ...
    print(f'Epoch {epoch}, Avg Loss: {avg_loss:.5f}, Avg ADE: {avg_ade:.5f}, Avg FDE: {avg_fde:.5f}, lr:{lr:.7f}')
